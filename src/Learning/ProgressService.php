...
final class ProgressService
{
    ...
    {
        $stmt=$this->pdo->prepare("SELECT l.skill,l.level,COUNT(*) available,COUNT(DISTINCT CASE WHEN a.status='completed' THEN a.activity_id END) completed,COALESCE(AVG(CASE WHEN a.status='completed' THEN a.score END),0) average_score,MAX(a.completed_at) latest_at FROM learning_activities l LEFT JOIN learning_attempts a ON a.activity_id=l.id AND a.member_id=? WHERE l.classroom_id=? AND l.status='ready' GROUP BY l.skill,l.level ORDER BY FIELD(l.skill,'reading','listening','speaking','writing'),FIELD(l.level,'basic','intermediate','advanced')");$stmt->execute([$memberId,$classroomId]);$rows=$stmt->fetchAll();$lowest=null;foreach($rows as $row)if($row['completed']<(int)$row['available']&&($lowest===null||(float)$row['average_score']<(float)$lowest['average_score']))$lowest=$row;
