...
/* ═══════════════════════════════════════
   FALLBACK QUESTIONS (Quiz)
═══════════════════════════════════════ */
const FALLBACK=[ 
  {u:3,q:"Change to active voice: 'The seeds are eaten by the bird.'",op:["A. The bird eats the seeds.","B. The bird is eating the seeds.","C. The bird ate the seeds.","D. The bird eat the seeds."],ans:"A",exp:"Passive present (are eaten) becomes Active present (eats) for singular subject.",cat:"Grammar",dif:"hard"},
  {u:1,q:"What is the main threat to the Bekantan's survival?",op:["A. Climate change","B. Habitat destruction","C. Natural predators","D. Overpopulation"],ans:"B",exp:"Deforestation and habitat destruction in mangrove forests threaten Bekantans.",cat:"Reading",dif:"medium"},
  ...
  {u:1,q:"Which of the following is a characteristic of a male bekantan?",op:["A. It has a small, flat nose.","B. It has a very large, fleshy nose.","C. It has no tail.","D. It has black fur."],ans:"B",exp:"Male bekantans are known for their distinctively large noses.",cat:"Reading",dif:"easy"},
  {u:1,q:"Identify the correct noun group:",op:["A. A monkey wild dangerous","B. A dangerous wild monkey","C. Wild dangerous a monkey","D. Monkey a dangerous wild"],ans:"B",exp:"Order: Article (A) + Opinion (dangerous) + Fact/Trait (wild) + Noun (monkey).",cat:"Grammar",dif:"hard"},
  {u:1,q:"'Bekantans live in groups.' The word 'groups' can be replaced by...",op:["A. Herds","B. Troops","C. Flocks","D. Schools"],ans:"B",exp:"A group of monkeys or primates is called a troop.",cat:"Vocabulary",dif:"hard"},
  {u:1,q:"Fill in the blank: 'Bekantan ___ a reddish-brown coat.'",op:["A. have","B. has","C. having","D. is"],ans:"B",exp:"Singular subject (Bekantan) uses 'has' to show possession.",cat:"Grammar",dif:"easy"},
  {u:1,q:"What does the word 'endangered' mean in the context of Bekantans?",op:["A. Very dangerous to humans","B. At risk of becoming extinct","C. Growing in population","D. Found all over the world"],ans:"B",exp:"Endangered means a species is seriously at risk of extinction.",cat:"Vocabulary",dif:"medium"},
  {u:1,q:"Look at this sentence: 'The baby monkey holds onto its mother.' What is the possessive adjective?",op:["A. holds","B. onto","C. its","D. baby"],ans:"C",exp:"'Its' shows possession belonging to the baby monkey.",cat:"Grammar",dif:"medium"},
  {u:1,q:"Bekantans are endemic to the island of Borneo. This means they are...",op:["A. Only found in Borneo","B. Originally from Africa","C. Can survive anywhere","D. Kept in zoos globally"],ans:"A",exp:"Endemic means native and restricted to a certain place.",cat:"Vocabulary",dif:"medium"}
];

/* ═══════════════════════════════════════
   SPEAKING FALLBACK (Modul Speaking Terpisah)
═══════════════════════════════════════ */
const SPEAKING_FALLBACK = [
  // Unit 1 – Bekantan
  {u:1,phrase:"The bekantan has a large distinctive bulbous reddish nose.",tips:"Tekankan 'bul-bous' dan 'dis-tinc-tive'. Bunyikan /z/ di 'nose'.",dif:"easy"},
  {u:1,phrase:"Bekantans are excellent swimmers with partially webbed feet.",tips:"Fokus pada 'swim-mers' dan 'web-bed'. Stress pada 'ex-cel-lent'.",dif:"medium"},
  {u:1,phrase:"Its bulbous nose makes the bekantan very easy to recognize.",tips:"'Bul-bous' harus jelas, jangan 'bul-bus'.",dif:"hard"},
  {u:1,phrase:"The proboscis monkey lives in the mangrove forests of Kalimantan.",tips:"'Proboscis' diucapkan pro-bos-kis.",dif:"medium"},
  // Unit 2 – Orangutan & Gorilla
  {u:2,phrase:"Orangutans have long orange-red arms and live in trees.",tips:"'Or-ang-u-tans' – stress pada suku kata kedua.",dif:"easy"},
  {u:2,phrase:"A gorilla has black fur and is much heavier than an orangutan.",tips:"Bandingkan 'heav-i-er' dengan jelas.",dif:"medium"},
  {u:2,phrase:"Orangutans have longer arms than gorillas.",tips:"'Long-er' harus jelas perbedaannya.",dif:"hard"},
  {u:2,phrase:"The silverback gorilla leads his family group.",tips:"'Sil-ver-back' dan possessive 'his'.",dif:"medium"},
  // Unit 3 – Indonesian Birds
  {u:3,phrase:"Cendrawasih is found in Papua and has spectacular plumage.",tips:"Passive voice: 'is found'. 'Spec-tac-u-lar'.",dif:"easy"},
  {u:3,phrase:"Helmeted hornbills are hunted for their solid casques.",tips:"'Are hunted' – passive voice jelas.",dif:"medium"},
  {u:3,phrase:"The Bali Starling is bred in captivity to prevent extinction.",tips:"'Is bred' dan 'pre-vent ex-tinc-tion'.",dif:"hard"},
  {u:3,phrase:"These beautiful birds are protected by Indonesian law.",tips:"'Are pro-tect-ed' – passive voice.",dif:"medium"}
];
...
function buildTeamGrid(){
  ...
  for(let i=0;i<tcnt;i++){
    const w=document.createElement('div');w.className='team-wrap';
    w.innerHTML=`<div class="tdot" style="background:${COLORS[i]}"></div><div class="fi"><input type="text" id="tn${i}" value="${def[i]}" placeholder="Nama kelompok ${i+1}" maxlength="18"></div>`;
    g.appendChild(w);
  }
}
function togUnit(u){
...
function startGame(){
  ...
  for(let i=0;i<tcnt;i++){
    const nm=(document.getElementById(`tn${i}`)?.value||`Kelompok ${String.fromCharCode(65+i)}`).trim();
...
function buildSB(){
  ...
  G.teams.forEach((t,i)=>{
    const d=document.createElement('div');d.className='tsc'+(i===G.tidx?' cur':'');d.id=`tsc${i}`;d.style.color=t.cl;
    d.innerHTML=`<div class="tsc-nm">${t.nm}</div><div class="tsc-sc" id="tss${i}">0</div><div class="tsc-st" id="tst${i}"></div>`;
    sb.appendChild(d);
  });
}
function updSB(){
...
function fallbackQ(){ 
  const avail=FALLBACK.filter(q=>G.units.includes(q.u));
  const unused=avail.filter(q=>!G.hist.some(h=>h.q===q.q));
  const pool=unused.length?unused:avail;
  return pool[Math.floor(Math.random()*pool.length)];
}
function fallbackSpeaking(){
  const avail=SPEAKING_FALLBACK.filter(q=>G.units.includes(q.u));
  const unused=avail.filter(q=>!G.hist.some(h=>h.phrase===q.phrase));
  const pool=unused.length?unused:avail;
  return pool[Math.floor(Math.random()*pool.length)];
}
...
async function nextTurn(){
  ...
  try{
    ...
    G.rnd--;
    document.getElementById('qarea').innerHTML=`<div class="load-q"><div class="load-txt">Materi belum dapat dibuat.<br><small>${error.message||'Pastikan guru telah memilih RPP aktif dan konfigurasi Gemini tersedia.'}</small><br><br><button class="btn btn-primary" onclick="nextTurn()">Coba Lagi</button></div></div>`;
    return;
  }
...
function showLoad(){
  const team=G.teams[G.tidx];
  const txt = G.mode==='quiz' 
    ? `🤖 AI menyiapkan pertanyaan untuk <b style="color:${team.cl}">${team.nm}</b>…` 
    : `🗣️ AI menyiapkan latihan pronunciation untuk <b style="color:${team.cl}">${team.nm}</b>…`;
  document.getElementById('qarea').innerHTML=`<div class="load-q"><div class="spin-ring"></div><div class="load-txt">${txt}</div></div>`;
}
...
function showQ(q){ 
  ...
  const ltrs=['A','B','C','D'];
  const opts=q.op.map((o,i)=>`<button class="opt sl" data-l="${ltrs[i]}" onclick="pick('${ltrs[i]}')" style="animation-delay:${.05+i*.07}s">
    <span class="opt-lt">${ltrs[i]}</span><span>${o.replace(/^[A-D]\.\s*/,'')}</span>
  </button>`).join('');
  document.getElementById('qarea').innerHTML=`
    <div class="q-meta sl">
      <span class="badge ${cc}">${q.cat}</span>
      <span class="badge bg-unit">${ICONS[q.u]||'📚'} Unit ${q.u}</span>
      <span class="badge ${dc}">${q.dif}</span>
    </div>
    <div class="gc qcard sl" style="animation-delay:.05s">
      <div class="qtxt">${q.q}</div>
    </div>
    <div class="opts sl" id="optsWrap" style="animation-delay:.1s">${opts}</div>
    <div class="exp-box" id="expBox">💡 <b>Penjelasan:</b> ${q.exp||''}</div>
  `;
}
...
function showSpeakingTask(task){
  ...
  document.getElementById('qarea').innerHTML=`
    <div class="q-meta sl">
      <span class="badge bg-pron">🗣️ Pronunciation</span>
      <span class="badge bg-unit">${ICONS[task.u]||'📚'} Unit ${task.u}</span>
      <span class="badge ${dc}">${task.dif}</span>
    </div>
    <div class="gc qcard sl" style="animation-delay:.05s">
      <div class="qtxt">Ucapkan kalimat berikut dengan jelas dan benar:</div>
      <div class="speak-phrase">${task.phrase}</div>
      <div style="font-size:0.9rem;color:var(--muted)"><b>Tips AI:</b> ${task.tips||'Ucapkan dengan intonasi alami'}</div>
    </div>
    <div style="text-align:center;margin-top:30px">
      <button onclick="startPronunciation()" class="btn btn-primary speak-btn">
        🗣️ Mulai Berbicara<br><small>AI akan nilai pronunciation secara real-time</small>
      </button>
    </div>
    <div id="speakResult" style="display:none;margin-top:20px"></div>
  `;
}
...
═══════════════════════════════════════ */
function pick(l){ 
  if(G.done)return;G.done=true;clrTimer();
  ...
  let pts=0,msg='';
  if(ok){
    const spd=G.timer?Math.round((G.left/TMAX)*50):0;
    const dm={easy:100,medium:150,hard:200}[q.dif]||100;
    G.streaks[tm]++;
    const skb=Math.max(0,(Math.min(G.streaks[tm],5)-1)*25);
    pts=dm+spd+skb;
    G.scores[tm]+=pts;G.correct[tm]++;
    msg=`+${pts} poin`+(spd>0?` ⚡+${spd}`:'')+(skb>0?` 🔥+${skb}`:'');
    MX.sfxOk();confetti();
  } else {
    G.streaks[tm]=0;
...
function startPronunciation(){
  ...
  resultDiv.innerHTML = `<div class="load-q" style="padding:30px"><div class="spin-ring" style="width:48px;height:48px"></div><div class="load-txt">🎙️ AI sedang mendengarkan...<br>Ucapkan kalimat di atas dengan jelas</div></div>`;

  const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
  if(!SpeechRecognition){
    resultDiv.innerHTML = `<div class="gc" style="padding:20px;color:#ef4444">Browser Anda tidak mendukung Speech Recognition. Gunakan Chrome/Edge.</div>`;
  ...
  recognition.onerror = (event) => {
    resultDiv.innerHTML = `<div class="gc" style="padding:20px;color:#ef4444">Error mic: ${event.error}. Coba lagi.</div>`;
...
function evaluatePronunciation(transcript, target, confidence){
  const sim = 1 - (levenshtein(transcript.toLowerCase().trim(), target.toLowerCase().trim()) / Math.max(transcript.length, target.length));
  let score = Math.round(sim * 100 * Math.min(confidence, 1));

  const tm = G.teams[G.tidx].nm;
  const pts = Math.max(30, Math.round(score * 1.8)); 
  G.scores[tm] += pts;
  G.streaks[tm]++;
  G.correct[tm]++;
  ...
  resultDiv.innerHTML = `
    <div class="gc" style="padding:22px">
      <div style="font-size:1.1rem;margin-bottom:12px">Kamu mengucapkan:</div>
      <div style="background:rgba(255,255,255,.1);padding:14px;border-radius:12px;font-size:1.15rem">"${transcript}"</div>
      <div style="margin:18px 0;font-size:2.4rem;font-weight:700;color:${score>=75?'#10b981':'#f59e0b'}">Skor AI: ${score}/100</div>
      <div style="color:#10b981;font-weight:600">+${pts} poin untuk ${tm}</div>
      ${G.curQ.tips?`<div style="margin-top:14px;font-size:0.9rem"><b>Tips:</b> ${G.curQ.tips}</div>`:''}
    </div>
  `;
  ...
  showFbk(true, tm, pts, `Pronunciation AI Score: ${score}`, '');
...
function usePU(type){ 
  ...
  if(type === '50'){
    ...
    updSB();
    MX.sfxOk();
    showFbk(true, team, pts, `Skip Dipakai!`, G.curQ.ans);
...
function endGame(){
  ...
  const sorted=[...G.teams].sort((a,b)=>G.scores[b.nm]-G.scores[a.nm]);
  ...
  sorted.forEach((t,i)=>{
    ...
    const rnkIco=['🥇','🥈','🥉'][i]||(i+1)+'';
    const row=document.createElement('div');row.className='lb-row gc sl';row.style.animationDelay=i*.08+'s';
    const pct=G.total>0?Math.round(G.correct[t.nm]/Math.ceil(G.total/G.teams.length)*100):0;
    row.innerHTML=`<div class="lb-rnk ${rnkCls}">${rnkIco}</div><div class="lb-dot" style="background:${t.cl}"></div><div class="lb-nm">${t.nm}</div><div class="lb-rt"><div class="lb-sc" style="color:${t.cl}">${G.scores[t.nm].toLocaleString()}</div><div class="lb-cr">${G.correct[t.nm]} benar / latihan</div></div>`;
    lb.appendChild(row);
  });
